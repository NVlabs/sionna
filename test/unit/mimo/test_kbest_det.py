...
from sionna.phy.mimo import KBestDetector, MaximumLikelihoodDetector
from sionna.phy.mapping import Constellation, Mapper, BinarySource, QAMSource, PAMSource
from sionna.phy.utils import compute_ser, compute_ber, ebnodb2no, sim_ber
from sionna.phy.channel import FlatFadingChannel
from sionna.phy.fec.ldpc.encoding import LDPC5GEncoder
from sionna.phy.fec.ldpc.decoding import LDPC5GDecoder

class MIMODetectionBER(Block):
    ...
    def __init__(self,
        ...
        self._encoder = LDPC5GEncoder(self._k, self._n, precision=precision)
        self._mapper = Mapper("qam", self._num_bits_per_symbol, precision=precision)
        ...
        if detector=="kbest":
            ...
            kbest_detector = KBestDetector("bit", num_tx, k,"qam", num_bits_per_symbol, use_real_rep=False, hard_out=not coded, precision=precision)
            self._detector = kbest_detector
        elif detector=="ml":
            ml_detector = MaximumLikelihoodDetector("bit", "maxlog", num_tx, "qam", num_bits_per_symbol, hard_out=not coded, precision=precision)
            self._detector = ml_detector
        self._decoder = LDPC5GDecoder(self._encoder, hard_out=True, precision=precision)
        self._coded = coded
    ...
    def call(self, batch_size, ebno_db):
        ...
        no =  tf.cast(self._num_tx, self.rdtype)*(10.0**(-ebno_db/10.0))
        y, h = self._channel(x, no)
        s = tf.cast(no*tf.eye(self._num_rx_ant), self.cdtype)
...
def test_wrong_parameters():
    ...
    with pytest.raises(AssertionError):
        ...
        KBestDetector("bit", 4, 16,
                        constellation=Constellation("pam", 4, precision="single"),
                        precision="double")

    with pytest.raises(AssertionError):
        "Wrong constellation dtype"
        KBestDetector("bit", 4, 16,
                        constellation=Constellation("pam", 4, precision="double"))

def test_init_complex_rep():
    num_bits_per_symbol = 4
    num_tx = 4
    k = 16
    ...
    assert detector._num_streams == num_tx
    assert detector._num_symbols == 2**num_bits_per_symbol
    assert k == detector._k
    assert np.allclose(np.var(detector._constellation), 1.0)
    ...
    with pytest.warns(Warning):
        detector = KBestDetector("bit", num_tx, 2*k_max,
                        constellation_type="qam",
                        num_bits_per_symbol = 4)
        assert detector._k == k_max

def test_wrong_constellation_for_real_rep():
...
def test_init_real_rep():
    ...
    detector = KBestDetector("bit", num_tx, k,
                        ...
                        num_bits_per_symbol=num_bits_per_symbol,
                        use_real_rep=True)
    assert detector._num_streams == 2*num_tx
    assert detector._num_symbols == 2**(num_bits_per_symbol//2)
    assert k == detector._k
    assert np.allclose(np.var(detector._constellation), 0.5)
...
def test_symbol_errors_complex_rep(num_bits_per_symbol):
...
@pytest.mark.parametrize("num_bits_per_symbol", [2,4,6,8])
def test_symbol_errors_real_rep(num_bits_per_symbol):
...
@pytest.mark.parametrize("num_bits_per_symbol", [1,2,3,4])
def test_symbol_errors_pam(num_bits_per_symbol):
...
@pytest.mark.parametrize("num_bits_per_symbol", [2,4,6,8])
def test_bit_errors_complex_rep(num_bits_per_symbol):
...
@pytest.mark.parametrize("num_bits_per_symbol", [2,4,6,8])
def test_bit_errors_real_rep(num_bits_per_symbol):
...
@pytest.mark.parametrize("num_bits_per_symbol", [1,2,3,4])
def test_bit_errors_pam(num_bits_per_symbol):
    """Test that we get no bit error using the real-valued representation"""
    num_tx = 4
    num_rx_ant = 7
    batch_size = 100
...
@pytest.mark.parametrize("real_rep", [True, False])
@pytest.mark.parametrize("ebno_db", [-20,-10,0,10,20,30,50])
def test_llr_against_ml_qam(num_bits_per_symbol, real_rep, ebno_db):
    num_tx = 3
    num_rx_ant = 8
    batch_size = 100
    ...
    k = (2**num_bits_per_symbol)**num_tx
    assert fun(ebno_db, num_bits_per_symbol, k, real_rep)

@pytest.mark.parametrize("num_bits_per_symbol", [1,2,3,4])
@pytest.mark.parametrize("ebno_db", [-20,-10,0,10,20,30,50])
def test_llr_against_ml_pam(num_bits_per_symbol, ebno_db):
    num_tx = 3
    num_rx_ant = 8
    batch_size = 100
    ...
    k = (2**num_bits_per_symbol)**num_tx
    assert fun(ebno_db, num_bits_per_symbol, k)

@pytest.mark.usefixtures("only_gpu")
def test_e2e_uncoded_ber_vs_ml():
    ...
    assert np.allclose(kbest_ber, ml_ber, atol=1e-3)

@pytest.mark.usefixtures("only_gpu")
def test_e2e_coded_ber_vs_ml():
    ...
    assert np.allclose(kbest_ber, ml_ber, rtol=0.1)
