...
import itertools
from sionna.phy.fec.utils import pcm2gm, int_mod_2, make_systematic
...
class OSDecoder(Block):
    ...
    This block implements the OSD algorithm as proposed in [Fossorier]_ and,
    ...
    is_pcm: `bool`, (default `False`)
        If `True`,  ``enc_mat`` is interpreted as parity-check matrix.

    encoder: Block | None
        Sionna block that implements a FEC encoder.
    ...
    def __init__(self,
        ...
        if enc_mat is not None:
            ...
            if isinstance(enc_mat, np.ndarray):
                if not np.array_equal(enc_mat, enc_mat.astype(bool)):
                    raise TypeError('PC matrix must be binary.')
        ...
        if encoder is not None:
            ...
            self._gm = tf.cast(tf.squeeze(encoder(u), axis=0), self.rdtype)
        else:
            if enc_mat is None:
                msg = "enc_mat cannot be None if no encoder is provided."
            ...
            if is_pcm:
                ...
                gm = enc_mat
            self._gm = tf.constant(gm, dtype=self.rdtype)
    ...
    def _find_min_dist(self, llr_ch, ep, gm_mrb, c):
        ...
        # used to be faster in older tf versions
    ...
    def _find_mrb(self, gm):
        ...
        for idx_c in tf.range(self._k):
            ...
            rz = tf.zeros((bs, 1), dtype=self.rdtype)
