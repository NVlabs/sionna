...
class PolarEncoder(Block):
    ...
    def call(self, bits):
        ...
        c = tf.zeros([self._n, tf.shape(u)[0]], u.dtype)
        ...
        x_nan = tf.zeros([tf.shape(c)[0] ,1], c.dtype)
        ...
        return tf.cast(c_reshaped, self.rdtype)

class Polar5GEncoder(PolarEncoder):
    # pylint: disable=line-too-long
    """5G compliant Polar encoder including rate-matching following [3GPPTS38212]_ for the uplink scenario (`UCI`) and downlink scenario (`DCI`).
    ...
    channel_type: 'uplink' (default) | 'downlink'
        Can be 'uplink' or 'downlink'.

    verbose: `bool`, (default `False`)
        If `True`,  rate-matching parameters will be printed.
    ...
    def __init__(self,
        ...
        self._enc_crc = CRCEncoder(crc_degree, precision=precision)

        # Init super-class (PolarEncoder)
        super().__init__(frozen_pos, n_polar, precision=precision, **kwargs)
    ...
    def _init_rate_match(self, k_target, n_target):
        ...
        if k_polar > n_target:
            msg = "Device is not expected to be configured " \
