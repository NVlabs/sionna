...
"""Transport block encoding functions for the NR (5G) module of Sionna PHY"""
...
class TBEncoder(Block):
    ...
    def tb_size(self):
        r"""
        `int` : Effective number of information bits per TB.
        Note that (if required) internal zero padding can be
        applied to match the request exact ``target_tb_size``.
        """
    ...
    def k_padding(self):
        """
        `int` : Number of zero padded bits at the end of the TB"""
        return self._k_padding

    @property
    def n(self):
        """
        `int` : Total number of output bits
        """
    ...
    def coderate(self):
        """
        `float` : Effective coderate of the TB after rate-matching including
        overhead for the CRC
        """
    ...
    def num_tx(self):
        """
        `int` : Number of independent streams
        """
        return self._num_tx
    ...
    def call(self, inputs):
        ...
        c_scr = tf.cast(c_scr, self.rdtype)
