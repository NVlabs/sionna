...
class LayerMapper(Block):
    ...
    inputs: [...,n], or [[...,n1], [...,n2]], `tf.complex`
        Sequence of symbols to be mapped. If
...
class LayerDemapper(Block):
    ...
    Parameters
    ----------
    layer_mapper: :class:`~sionna.phy.nr.LayerMapper`
        Associated LayerMapper

    num_bits_per_symbol: `int`, (default 1)
    ...
    def build(self, input_shapes):
        ...
        assert input_shapes[-2]==num_layers, \
            "Invalid input dimension: input shape must be [...,num_layers,n]."

        assert input_shapes[-1]%self._num_bits_per_symbol==0, \
