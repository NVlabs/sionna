...
from sionna.phy.utils import expand_to_rank, flatten_last_dims, flatten_dims, \
                             split_dim, insert_dims, hard_decisions
from sionna.phy.mapping import Constellation, SymbolLogits2LLRs, \
                               LLRs2SymbolLogits, PAM2QAM, Demapper, \
                               SymbolDemapper, SymbolInds2Bits, \
                               SymbolLogits2Moments
from sionna.phy.mimo.utils import complex2real_channel, whiten_channel, \
                                  List2LLR, List2LLRSimple, \
                                  complex2real_matrix, complex2real_vector, \
                                  real2complex_vector
...
class LinearDetector(Block):
    ...
    such as :func:`~sionna.phy.mimo.lmmse_equalizer`, and a :class:`~sionna.phy.mapping.Demapper`.
    ...
    : [..., num_streams, num_points], `tf.float` or [..., num_streams], `tf.int32`
...
class MaximumLikelihoodDetector(Block):
    ...
    Optionally, prior information of the transmitted signal :math:`\mathbf{x}`
    can be provided, either as LLRs on the bits mapped onto :math:`\mathbf{x}` or as logits on the individual
    ...
    **ML detection of symbols:**
    ...
    When hard decisions are requested, this block returns for the :math:`k` th stream
    ...
    prior : `None` (default) | [...,num_streams,num_bits_per_symbol] or [...,num_streams,num_points], `tf.float`
    ...
    def __init__(self,
        ...
        self._vecs = tf.cast(vecs, self.cdtype)
    ...
    def call(self, y, h, s, prior=None):
...
class KBestDetector(Block):
    ...
    see :func:`~sionna.phy.mimo.complex2real_channel`. We assume in the sequel the complex-valued
    representation. Then, the channel is whitened using :func:`~sionna.phy.mimo.whiten_channel`:
    ...
    **LLR Computation**
    ...
    is :class:`~sionna.phy.mimo.List2LLRSimple`. Custom solutions can be provided.
    ...
    def __init__(self,
        ...
        if constellation is not None:
            assert constellation.precision==self.precision, \
        ...
        if self._use_real_rep:
            ...
            self._constellation = tf.cast(c.points, self.rdtype)
            ...
            c = constellation = Constellation.check_or_create(
                                    constellation_type=constellation_type,
                                    num_bits_per_symbol=num_bits_per_symbol,
                                    constellation=constellation,
                                    precision=precision)
    ...
    def list2llr(self):
        """
        :class:`~sionna.phy.mimo.List2LLR` : Set/get the function to be used to compute
            LLRs from a list of candidate solutions
        """
    ...
    def build(self, *input_shapes):
        assert input_shapes[1][-2]>=input_shapes[1][-1], \
                "The number of receive antennas cannot be smaller \
                 than the number of streams"

    def call(self, y, h, s):
        # Flatten the batch dimensions
        batch_shape = [s if s is not None else -1 for s in y.shape[:-1]]
...
class EPDetector(Block):
    ...
    This block implements Expectation Propagation (EP) MIMO detection as described
    ...
    The channel model is first whitened using :func:`~sionna.phy.mimo.whiten_channel`
    and then converted to its real-valued equivalent,
    see :func:`~sionna.phy.mimo.complex2real_channel`, prior to MIMO detection.

    The computation of LLRs is done by converting the symbol logits
    that naturally arise in the algorithm to LLRs using
    :func:`~sionna.phy.mapping.PAM2QAM`. Custom conversions of symbol logits to LLRs
    ...
    def __init__(self,
        ...
        if self._output=="symbol":
            self._pam2qam = PAM2QAM(num_bits_per_symbol,
                                    hard_out,
                                    precision=precision)
        else:
        ...
        self._num_bits_per_symbol = num_bits_per_symbol//2
        points = Constellation("pam", int(self._num_bits_per_symbol),
                               precision=precision)()

        # Scale constellation points to half the energy because QAM is assumed
        self._points = tf.cast(points/np.sqrt(2.0), self.rdtype)

        # Average symbol energy
        self._es = tf.constant(np.var(self._points), self.rdtype)
    ...
    def compute_v_x(self, v_obs, x_obs):
        ...
        logits = -tf.pow(x_obs-points, 2) / (tf.cast(2, self.rdtype)*v_obs)
...
class MMSEPICDetector(Block):
    ...
    This block implements the MMSE PIC detector, as proposed in [CST2011]_.
    ...
    demapping_method : "maxlog" (default) | "app"
        Demapping method to be used
    ...
    def __init__(self,
        ...
        self._bit_demapper = Demapper(demapping_method=demapping_method,
                                      constellation=self._constellation,
                                      with_prior=True,
                                      precision=precision)

    def call(self, y, h, s, prior):
        ...
        def mmse_pic_self_iteration(llr_d, llr_a, it):
            ...
            llr_d = tf.reshape(self._bit_demapper(x_hat, no_eff, llr_a),
                                llr_shape)
        ...
        it = tf.constant(0)
        null_prior = tf.zeros(llr_shape, dtype=self.rdtype)
