...
#
"""Indoor factory (InF) channel model from 3GPP TR 38.901 specification"""
...
class InF(SystemLevelChannel):
    r"""
    Indoor factory (InF) channel model. It implements the indoor-factory
    scenario from Section 7 of 3GPP TR 38.901 :cite:p:`TR38901V1920`. The
    ``factory_scenario`` parameter selects one of the InF-SL (sparse clutter,
    low BS height), InF-DL (dense clutter, low BS height), InF-SH (sparse
    clutter, high BS height), InF-DH (dense clutter, high BS height), or InF-HH
    (high transmitter and high receiver) sub-scenarios.

    For InF-SL, InF-DL, InF-SH, and InF-DH, omitted hall and clutter values
    use the calibration assumptions from Table 7.8-7 of
    :cite:p:`TR38901V1920`. InF-HH is not included in that
    table. Its defaults ``(300, 150, 10)`` m, ``0.0``, ``1.0`` m, and
    ``0.0`` m for hall dimensions, clutter density, clutter size, and clutter
    height are implementation assumptions, not standardized calibration
    values. Pass explicit values for a particular InF-HH layout.

    Internally, this channel constructs an
    :class:`~sionna.phy.channel.tr38901.InFScenario`, which stores the factory
    layout, versioned channel parameters, and topology-dependent state. Most
    users should instantiate :class:`InF`; the scenario class is exposed for
    lower-level use with components such as
    :class:`~sionna.phy.channel.tr38901.LSPGenerator`.

    Setting up an InF model requires configuring the network topology, i.e.,
    the UT and base-station locations, UT velocities, etc. This is achieved using the
    :meth:`~sionna.phy.channel.tr38901.InF.set_topology` method. If
    ...
    the InF model. TR 38.901 indoor-factory topologies can be generated with
    :func:`~sionna.sys.gen_tr38901_indoor_factory_topology`.
    ...
    .. note::
        ...
        InF supports blockage model B only. The Model A parameter tables in
        Section 7.6.4.1 of :cite:p:`TR38901V1920` define
        blocker geometry and spatial-correlation distances for UMi, UMa, SMa,
        RMa, and InH, but not InF. Model B is geometry-driven and Table
        7.6.4.2-5 explicitly includes InF blockers such as humans, automated
        guided vehicles, and industrial robots. Requiring explicit Model B
        screens avoids introducing unspecified Model A parameters for InF.
    ...
    :param enable_spatial_consistency: If `True`, additionally generate
        ...
        `False`.
    :param enable_blockage: If `True`, apply blockage model B according to
        Section 7.6.4.2 of :cite:p:`TR38901V1920`. Explicit
        screen centres, widths, and heights are then required. The optional,
        on-demand temporal variability of blockage is currently not supported.
        See :ref:`tr38901-blockage`. Defaults to `False`.
    :param blockage_screen_centers: Blockage model B screen centres [m]. Shape
    ...
    .. code-block:: python
        ...
        import torch
        from sionna.phy.channel.tr38901 import InF, PanelArray
        ...
        channel_model = InF(carrier_frequency=carrier_frequency,
                            ut_array=ut_array,
                            bs_array=bs_array,
                            direction='downlink',
                            factory_scenario='SH',
        ...
        topology = gen_tr38901_indoor_factory_topology("SH",
                                                       batch_size=1,
                                                       num_ut=4,
                                                       device=device)
    ...
    def __init__(
        ...
        scenario = InFScenario(
            ...
            bs_array,
            direction,
            hall_dimensions=hall_dimensions,
            clutter_density=clutter_density,
            clutter_size=clutter_size,
    ...
    def set_topology(
        ...
        loss is not applied by the InF model.
        ...
        :param distance_2d_in: Not applicable to native indoor InF links;
        ...
        :param ut_spatial_region_ids: Optional integer correlation-region
            ...
            uses the same partition when applicable; InF supports model B
            only. InF uses one region by default. Shape [num_ut] or
            [batch size, num_ut].
        """
